...
public interface OrgDepAndMemRepository extends JpaRepository<OrgDepartmentAndOrgMember, Integer> {
    ...
    @Query("select od from OrgDepartmentAndOrgMember od Left join Fetch od.memberList m left join fetch m.orgPosition where od.depDeleteStatus = 'N' and(od.refDepCode is not null and od.refDepCode <> 1)")
